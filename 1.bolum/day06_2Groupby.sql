...
select ulke, count(ad) as calisan_sayisi from personel group by ulke;
...
select cinsiyet, avg(maas) as ortalama_maas from personel group by cinsiyet;
-- maas ortalamasini yas olarak sorguladik
select yas, avg(maas) as ortalama_maas from personel group by yas;

-- SORU05: Personelin, ulkelere ve şehirlere göre gruplayarak sorgulayın
select ulke,sehir from personel group by ulke,sehir;

-- SORU06: Personelin, ulkelere ve şehirlere göre calışan sayısını sorgulayın.
select ulke,sehir, count(ad) from personel group by ulke,sehir;

-- SORU07: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select ulke cinsiyet, count(cinsiyet), round(avg(yas),1) from personel group by ulke,cinsiyet;                                               

-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.
select ulke,cinsiyet, count(cinsiyet), round(avg(yas),1) 
...
select ulke,cinsiyet, count(cinsiyet) as kisi, round(avg(yas),1) as ortalama_yas
from personel where maas>30000 group by ulke,cinsiyet order by ortalama_yas desc;
...
--  gruplandıran ve isme göre ters sırasıda listeyen sorguyu yazınız.
...
select isim,urun_adi, sum(urun_miktari) as toplam_deger from manav 
group by isim, urun_adi having toplam_deger >3 order by toplam_deger desc; 
...
select distinct urun_adi,urun_miktari from manav;
