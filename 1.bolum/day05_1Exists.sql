...
-----------------------------------------------------------------------------*/ 
select  urun_isim,musteri_isim from nisan where exists (select  urun_id from mart where mart.urun_id=nisan.urun_id)
select  urun_isim,musteri_isim from nisan where urun_id in (select urun_id from mart where mart.urun_id=nisan.urun_id)
    
/* SORU3: Her iki ayda ortak satilmayan ürünlerin URUN_ISIM'lerini ve   bu ürünleri
  NİSAN ayında satın alan MUSTERI_ISIM'lerini listeleyen bir sorgu yazınız. 
 -----------------------------------------------------------------------------*/
​select urun_isim,musteri_isim from nisan where not exists(select urun_id from mart where mart.urun_id=nisan.urun_id);
...
​
-- SORU4: ismi null olanları sorgula
