...
select * from calisanlar;
 ...
 update calisanlar
 set maas = (select maas * 1.2 from calisanlar where id = '1004')
 where id = '1004'

 -- 3) Maaşı ortalamanın altında olanlara %20 zam yapınız.  
update calisanlar 
set maas=maas*1.2 where  maas<(select avg(maas) from calisanlar);

-- 4) calisanların isim ve cocuk_sayisi'ni listeyen sorguyu yazınız.
select calisanlar.isim, aileler.cocuk_sayis from aileler left join calisanlar on aileler.id = calisanlar.id
...
select calisanlar.isim, calisanlar.maas + aileler.ek_gelir as toplam_gelir
from aileler left join calisanlar on aileler.id = calisanlar.id
...
select calisanlar.isim, (calisanlar.maas + aileler.ek_gelir) / (aileler.cocuk_sayisi + 2) as kisi_basi_gelir from aileler
left join calisanlar on aileler.id = calisanlar.id
