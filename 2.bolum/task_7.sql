...
select ograd,count(ograd)as kisi_sayisi from ogrenci group by ograd;
...
select sinif,cinsiyet,count(*) as adet from ogrenci group by sinif,cinsiyet;

-- Task 4: Her türden kaç tane kitap olduğunu listeleyiniz.
select turno, count(idno) from kitap group by turno;

-- Task 5: Her türdeki en fazla sayfa sayısı olan kitapları listeleyiniz.
select k.turno,i.kitap_turu,max(sayfasayisi) from kitap k,islemler i where k.turno=i.turno group by k.turno order by k.turno desc;

-- Task 6: Her türdeki en az sayfa sayısı olan kitapları listeleyiniz.
select turno, max(sayfasayisi) as enaz_sayfa from kitap group by turno;

-- Task 7***: Her türden kaç tane kitap olduğunu listeleyiniz.(Tür isimleri de olsun)
select kitap_turu, (select count(turno) from kitap where islemler.turno = kitap.turno) as kitap_adedi from islemler;
